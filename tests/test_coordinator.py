...
async def test_coordinator_init(hass: HomeAssistant, mock_config_entry):
    ...
    with patch("custom_components.enphase_production_toggle.coordinator.EnvoyClient") as mock_client_class:
...
async def test_coordinator_update_data_success(hass: HomeAssistant, mock_config_entry, mock_envoy_client):
    """Test successful data update."""
    with patch("custom_components.enphase_production_toggle.coordinator.EnvoyClient", return_value=mock_envoy_client):
        coordinator = EnphaseDataUpdateCoordinator(hass, mock_config_entry)
        
        # Mock successful responses
        mock_envoy_client.authenticate = AsyncMock()
        mock_envoy_client.get_production_status = AsyncMock(return_value={
            "is_producing": True,
            "current_power": 5000,
...
async def test_coordinator_update_data_auth_failure(hass: HomeAssistant, mock_config_entry, mock_envoy_client):
    ...
    with patch("custom_components.enphase_production_toggle.coordinator.EnvoyClient", return_value=mock_envoy_client):
        ...
        with pytest.raises(UpdateFailed, match="Error communicating with API: Auth failed"):
            await coordinator._async_update_data()


async def test_coordinator_update_data_get_status_failure(hass: HomeAssistant, mock_config_entry, mock_envoy_client):
    ...
    with patch("custom_components.enphase_production_toggle.coordinator.EnvoyClient", return_value=mock_envoy_client):
        ...
        mock_envoy_client.authenticate = AsyncMock()
        mock_envoy_client.get_production_status = AsyncMock(side_effect=Exception("Status failed"))
        
        with pytest.raises(UpdateFailed, match="Error communicating with API: Status failed"):
            await coordinator._async_update_data()


async def test_coordinator_client_property(hass: HomeAssistant, mock_config_entry, mock_envoy_client):
