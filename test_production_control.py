...
async def test_production_control():
    ...
    try:
        ...
        try:
            async with client._session.put(url1, headers=headers, json=data1, ssl=False) as response:
                print(f"PUT {url1}")
                print(f"Data: {data1}")
        ...
        try:
            async with client._session.put(url2, headers=headers, json=data2, ssl=False) as response:
                print(f"PUT {url2}")
                print(f"Data: {data2}")
                print(f"Status: {response.status}")
                text = await response.text()
        ...
        try:
            async with client._session.put(url2, headers=headers, json=data3, ssl=False) as response:
                print(f"PUT {url2}")
                print(f"Data: {data3}")
                print(f"Status: {response.status}")
                text = await response.text()
        ...
        try:
            async with client._session.get(url2, headers=headers, ssl=False) as response:
                print(f"GET {url2}")
                print(f"Status: {response.status}")
                text = await response.text()
                print(f"Response: {text[:500]}")
